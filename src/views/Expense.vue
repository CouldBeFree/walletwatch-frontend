<template>
  ...
  <v-row>
    <v-col
      ...
      md="4"
      lg="3"
      v-for="cat in getAllSubCategories"
    >
      <v-card
        class="mx-auto"
  ...
  <v-dialog v-model="dialogCreate" scrollable max-width="500px" width="400">
    <v-card>
      <v-card-title
        >Створити категорію для {{ getExpense.expense }}</v-card-title
      >
      <v-form @submit.prevent ref="form" v-model="valid" lazy-validation>
        <v-card-item>
          ...
          <v-text-field
            ...
            :append-icon="
              expenseItem.expense_category_icon || 'mdi-material-ui'
            "
          />
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn variant="flat" color="primary" @click="dialogCreate = false">
              Відмінити
            </v-btn>
            <v-btn
              @click="onSubmit"
  ...
  <v-dialog v-model="dialogRemove" scrollable max-width="500px" width="400">
    <v-card>
